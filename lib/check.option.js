...
var checkOptions = function (result, option) {
    ...
    if (option.insertSymbol) {
        var characters = option.range ? (types_1.symbols + option.range + types_1.symbols) :
            option.charset === "number" ? (types_1.numberSymbol) :
                option.charset === "alphabet" ? (types_1.alphabetSymbol) :
